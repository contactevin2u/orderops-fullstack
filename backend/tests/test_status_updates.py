...
def test_mark_cancelled_creates_adjustment():
    ...
    assert any(
        getattr(o, "code", "").endswith("-C") for o in db.added if isinstance(o, Order)
    )
...
def test_apply_buyback_creates_adjustment_with_discount():
    ...
    assert order.status == "RETURNED"
    adj = next(o for o in db.added if isinstance(o, Order) and o.code.endswith("-B"))
    ...
    assert order.paid_amount == Decimal("-90.00")
