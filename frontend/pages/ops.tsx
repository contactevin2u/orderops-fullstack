      ...
      <div className="card">
        ...
        <div className="row">
          ...
          <div className="col" style={{ display: "flex", alignItems: "center", gap: 8 }}>
            ...
            <Link className="btn secondary" href="/parse">
              Create from Parse
            </Link>
            <Link className="btn secondary" href="/reports/outstanding">
              Outstanding
            </Link>
            <Link className="btn secondary" href="/export">
              Export
            </Link>
          </div>
        </div>
        <ErrorBoundary fallback={<div style={{ opacity: 0.7 }}>Failed to load orders</div>}>
        ...
        <tbody>
          {items.map((o: any) => (
            <tr key={o.id}>
              ...
              <td style={{ textAlign: "right" }}>
                RM {Number(o.paid_amount || 0).toFixed(2)}
              </td>
              <td style={{ textAlign: "right" }}>
                RM {Number(o.balance || 0).toFixed(2)}
              </td>
            </tr>
          ))}
          {items.length === 0 && (
