...
export default function NewQuotationPage() {
  ...
  const [items, setItems] = React.useState<any[]>([
    { name: "", item_type: "OUTRIGHT", qty: 1, unit_price: "", monthly_amount: "" },
  ]);
  ...
  async function generateQuotation() {
    ...
    try {
      ...
      const response = await fetch("/_api/orders/quotation.pdf", {
        method: "POST",
        headers: {
      ...
      <div className="stack">
        ...
        <Card className="stack">
          <div className="stack">
            ...
            />
            <button className="btn secondary" onClick={onParse} disabled={busy || !rawText.trim()}>
              Parse
            </button>
          </div>
          <div className="row">
          ...
          <div className="stack">
            <label>Address</label>
            <textarea className="textarea" rows={2} value={custAddress} onChange={(e) => setCustAddress(e.target.value)} />
          </div>
          <div className="row">
            <div className="col">
              <label>Type</label>
            ...
            <tbody>
              {items.map((it, idx) => (
                <tr key={idx}>
                  ...
                  <td>
                    <input className="input" value={it.qty} onChange={(e) => updateItem(idx, "qty", e.target.value)} />
                  </td>
                  <td>
                    <input className="input" value={it.unit_price} onChange={(e) => updateItem(idx, "unit_price", e.target.value)} />
                  </td>
                  <td>
                    <input className="input" value={it.monthly_amount} onChange={(e) => updateItem(idx, "monthly_amount", e.target.value)} />
                  </td>
                  <td>
          ...
          <div className="row">
            ...
            <div className="col">
              <label>Return Delivery Fee</label>
              <input className="input" value={returnDeliveryFee} onChange={(e) => setReturnDeliveryFee(e.target.value)} />
            </div>
          </div>
          ...
          <div className="row">
            ...
            <div className="col">
              <label>Months</label>
              <input className="input" value={planMonths} onChange={(e) => setPlanMonths(e.target.value)} />
            </div>
            <div className="col">
              <label>Monthly Amount</label>
              <input className="input" value={planMonthly} onChange={(e) => setPlanMonthly(e.target.value)} />
            </div>
          </div>

          {err && <div style={{ color: "#ffb3b3" }}>{err}</div>}
          <div>
            <button className="btn" onClick={generateQuotation} disabled={busy || !custName || items.length === 0}>
