...
export default function DriverSchedulePage() {
  ...
  // Fetch daily drivers for selected date with actual schedule data
  const { data: dailyDrivers, isLoading: loadingDaily } = useQuery({
    queryKey: ['daily-drivers', selectedDate.format('YYYY-MM-DD')],
    queryFn: async () => {
      const response = await fetch(`/_api/driver-schedule/drivers/all?target_date=${selectedDate.format('YYYY-MM-DD')}`);
  ...
  const setScheduleMutation = useMutation({
    ...
    onSuccess: (data, variables) => {
      // Just refresh the data from the server to get the real state
